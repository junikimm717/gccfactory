// Package logging provides the single logger used by every gccfactory
// package. Every event goes to two places at once: a colorized human stream on
// stderr, and a machine-readable JSONL stream at
// dist/logs/runs/<ts>-<pid>/run.jsonl so a failed run can be replayed and
