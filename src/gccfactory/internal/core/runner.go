...
type Cmd struct {
	...
	Env []string
	// EnvAdd overlays variables onto the inherited environment.
	EnvAdd map[string]string
	// Args is the argv; Args[0] is the program.
	Args []string
	// Name labels the log file, e.g. "gcc-configure".
	Name string
}
...
// itself data (e.g. `gcc -dumpmachine`). It is logged exactly the same way.
...
func buildEnv(c Cmd) (env []string, explicit []string) {
	if c.Env != nil {
		env = append([]string(nil), c.Env...)
		explicit = append([]string(nil), c.Env...)
