...
type Job interface {
	// Name is the recipe family, e.g. "cross" or "canadian".
	Name() string
	// Slug is a filesystem-safe identifier, unique per job and stable across
	// recipe edits. It selects the artifact path, lock file and log directory.
	Slug() string
	// Deps are the jobs whose artifacts this job reads.
	Deps() []Job
	// KeyInputs is the recipe-specific contribution to the content key: recipe
	// version, every source sha256, every configure flag, every triple. It
...
// Manifest is the .gccfactory.json stamp inside a published artifact. Its
